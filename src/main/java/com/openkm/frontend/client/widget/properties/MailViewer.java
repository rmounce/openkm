                ...
                public void onSuccess(List<GWTDocument> result) {
                    for (final GWTDocument attach : result) {
                        ...
                        attachmentsPanel.add(WidgetUtil.getAttachmentWidget(attach, new AttachmentHandler() {
                            @Override
                            public void onAttachmentMoudeDown(MouseDownEvent event, GWTDocument attach, Button button) {
		...
		button.addMouseDownHandler(new MouseDownHandler() {
			...
			public void onMouseDown(MouseDownEvent event) {
				...
				if (buttonPressed == NativeEvent.BUTTON_RIGHT) {
					...
					int mouseX = event.getClientX();
					int mouseY = event.getClientY();
					Main.get().mainPanel.desktop.browser.tabMultiple.tabMail.mailViewer.attachmentMenuPopup.setPopupPosition(mouseX, mouseY);
					Main.get().mainPanel.desktop.browser.tabMultiple.tabMail.mailViewer.attachmentMenuPopup.show();
				}
			}
		});
		return button;
